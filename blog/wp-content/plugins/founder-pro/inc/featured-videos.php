...
function ct_founder_pro_add_video_meta_box() {
	...
	foreach ( $screens as $screen ) {
		...
		add_meta_box(
			'ct_founder_pro_video',
			__( 'Featured Video', 'founder-pro' ),
			'ct_founder_pro_video_callback',
			$screen,
			'normal',
			'high'
		);
...
add_action( 'add_meta_boxes', 'ct_founder_pro_add_video_meta_box' );

function ct_founder_pro_video_callback( $post ) {

	wp_nonce_field( 'ct_founder_pro_video', 'ct_founder_pro_video_nonce' );

	$video_url        = get_post_meta( $post->ID, 'ct_founder_pro_video_key', true );
	$youtube_title    = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_title', true );
	$youtube_related  = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_related', true );
	$youtube_logo     = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_logo', true );
	$youtube_captions = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_captions', true );

	// video preview
	echo '<div class="ct_founder_pro_video_preview_container" id="ct_founder_pro_video_preview_container">';
		echo '<label for="ct_founder_pro_video_url">';
			_e( 'Video Preview', 'founder-pro' );
		echo '</label> ';
		if ( $video_url ) {
			echo wp_oembed_get( esc_url( $video_url ) );
		}
		echo '<span class="loading">' . ct_founder_pro_loading_indicator_svg() . '</span>';
	echo '</div>';

	// video URL input
	echo '<div class="ct_founder_pro_video_input_container">';
		echo '<label for="ct_founder_pro_video_url">';
			_e( 'Add video URL:', 'founder-pro' );
		echo '</label> ';
		echo '<div>';
			echo '<input type="text" class="regular-text" id="ct_founder_pro_video_url" name="ct_founder_pro_video_url" value="' . esc_url( $video_url ) . '" />';
			echo ct_founder_pro_green_checkmark_svg();
		echo '</div>';
	echo '</div>';
	...
	if ( $post->post_type == 'post' ) :
		...
		$display_blog = get_post_meta( $post->ID, 'ct_founder_pro_video_display_key', true );

		if ( empty( $display_blog ) ) {
			$display_blog = "post";
		}

		echo '<div class="ct_founder_pro_video_display_container">';
			echo '<p>' . __( 'Choose where to display the video:', 'founder-pro' ) . '</p>';
			echo '<label for="ct_founder_pro_video_display_post">';
				echo '<input type="radio" name="ct_founder_pro_video_display" id="ct_founder_pro_video_display_post" value="post" ' . checked( $display_blog, "post", false ) . '>';
				_e( 'Post', 'founder-pro' );
			echo '</label> ';
			echo '<label for="ct_founder_pro_video_display_blog">';
				echo '<input type="radio" name="ct_founder_pro_video_display" id="ct_founder_pro_video_display_blog" value="blog" ' . checked( $display_blog, "blog", false ) . '>';
				_e( 'Blog', 'founder-pro' );
			echo '</label> ';
			echo '<label for="ct_founder_pro_video_display_both">';
				echo '<input type="radio" name="ct_founder_pro_video_display" id="ct_founder_pro_video_display_both" value="both" ' . checked( $display_blog, "both", false ) . '>';
				_e( 'Post & Blog', 'founder-pro' );
			echo '</label> ';
		echo '</div>';
	endif;
	...
	echo '<div class="ct_founder_pro_video_youtube_controls_container ' . esc_attr( $class ) . '">';
		echo '<p>' . __( 'Youtube controls', 'founder-pro' ) . '</p>';
		echo '<label for="ct_founder_pro_video_youtube_title">';
			echo '<input type="checkbox" name="ct_founder_pro_video_youtube_title" id="ct_founder_pro_video_youtube_title" value="1" ' . checked( '1', $youtube_title, false ) . '>';
			_e( 'Hide title', 'founder-pro' );
		echo '</label> ';
		echo '<label for="ct_founder_pro_video_youtube_related">';
			echo '<input type="checkbox" name="ct_founder_pro_video_youtube_related" id="ct_founder_pro_video_youtube_related" value="1" ' . checked( '1', $youtube_related, false ) . '>';
			_e( 'Hide related videos', 'founder-pro' );
		echo '</label> ';
		echo '<label for="ct_founder_pro_video_youtube_logo">';
			echo '<input type="checkbox" name="ct_founder_pro_video_youtube_logo" id="ct_founder_pro_video_youtube_logo" value="1" ' . checked( '1', $youtube_logo, false ) . '>';
			_e( 'Hide Youtube logo', 'founder-pro' );
		echo '</label> ';
		echo '<label for="ct_founder_pro_video_youtube_captions">';
			echo '<input type="checkbox" name="ct_founder_pro_video_youtube_captions" id="ct_founder_pro_video_youtube_captions" value="1" ' . checked( '1', $youtube_captions, false ) . '>';
			_e( 'Show Captions by Default', 'founder-pro' );
		echo '</label> ';
	echo '</div>';
}

function ct_founder_pro_add_oembed_callback() {
	...
	$response = $video_url ? wp_oembed_get( esc_url( $video_url ) ) : '';

	echo $response;

	die(); // this is required to return a proper result
}
add_action( 'wp_ajax_add_oembed', 'ct_founder_pro_add_oembed_callback' );

function ct_founder_pro_video_save_data( $post_id ) {

	global $post;

	if ( ! isset( $_POST['ct_founder_pro_video_nonce'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['ct_founder_pro_video_nonce'], 'ct_founder_pro_video' ) ) {
		return;
	}
	...
	if ( isset( $_POST['ct_founder_pro_video_url'] ) ) {

		$video_url = esc_url_raw( $_POST['ct_founder_pro_video_url'] );

		update_post_meta( $post_id, 'ct_founder_pro_video_key', $video_url );

		// save display option for posts only
		if ( $post->post_type == 'post' ) {

			if ( isset( $_POST['ct_founder_pro_video_display'] ) ) {

				$display_blog = esc_attr( $_POST['ct_founder_pro_video_display'] );

				if ( $display_blog == 'post' || $display_blog == 'blog' || $display_blog == 'both' ) {
					update_post_meta( $post_id, 'ct_founder_pro_video_display_key', $display_blog );
				}
			}
		}
	}
...
add_action( 'pre_post_update', 'ct_founder_pro_video_save_data' );

// green checkmark icon used in Post Video input
function ct_founder_pro_green_checkmark_svg() {
...
function ct_founder_pro_loading_indicator_svg() {
...
function ct_founder_pro_output_featured_video( $featured_image ) {

	global $post;
	$featured_video = get_post_meta( $post->ID, 'ct_founder_pro_video_key', true );
...
add_filter( 'ct_founder_featured_image', 'ct_founder_pro_output_featured_video' );

function ct_founder_pro_add_youtube_parameters( $html, $url, $args ) {
	...
	if ( ! empty( $featured_video ) ) {
		...
		if ( $url == $featured_video ) {
			...
			if ( strpos( $featured_video, 'youtube.com' ) || strpos( $featured_video, 'youtu.be' ) ) {
				...
				$youtube_title   = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_title', true ) ? 0 : 1;
				$youtube_related = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_related', true ) ? 0 : 1;
				$youtube_logo    = get_post_meta( $post->ID, 'ct_founder_pro_video_youtube_logo', true ) ? 0 : 1;
...
add_filter( 'oembed_result', 'ct_founder_pro_add_youtube_parameters', 10, 3 );